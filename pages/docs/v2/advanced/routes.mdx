...
export const meta = {
  ...
  lastEdited: '2019-09-09T21:08:11.000Z'
}
...
The `routes` array is usually processed before attempting a match. For example, this allows us to send a 404 for specific route patterns:
...
This would return a 404 for `/build/stats`. An advanced usage of this pattern can be:
...
In the snippet above we give precedence to our files in the filesystem - which you can see by appending /\_src to your deployment URL.

### Fallback to `index.html`

A common pattern with single-page applications is routing everything to a single file and the browser parses the path. Most SPAs have assets to serve as well, so we should handle the filesystem before rewriting the path:
...
It's important to note that serverless functions are also handled in the filesystem.

### Intercept routes after and before the filesystem

Say the directory structure of a user's project looks like this:
...
If we want to create a route that doesn't interfere with the other files in the filesystem, we should add `{ "handle": "filesystem" }` before:
...
This is the equivalent of writing except when you add a new file, you don't need to change any config:
...
When you need to rewrite the URL without matching, you should use `continue`

````json
    {
      "version": 2,
      "routes": [
        { "src": "/test", "headers": { "Cache-Control": "max-age: 600" }, "continue": true },
...
When requesting `/test`, it will look for a response at `/src/public/test`. Any routes that follow the `dest` + `continue` route will match against the new path. For example, `/test` would match all 3 routes.

### Common Regex

When using the `.` character, you should keep in mind in regex, it matches anything. A literal `.` is denoted `\\.`. Slashes do not need to be escaped because the regular expression is enclosed in quotes.
...
Anything that isn't a slash will match in `$1`. This allows `/blog/post` to work, while not allowing `/blog/post/edit`.
...
In order to make `/` use `www/index.html`, we have to create a route:
